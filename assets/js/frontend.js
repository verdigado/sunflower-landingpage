...
( () => {
    ...
    const groupBreakpoint = 950; // px (nur für Group-Slider)
    const tarifeBreakpoint = 1201; // px (Tarife sollen bis <1200 sliden)
    ...
    function getTrackType( trackEl ) {
        ...
        const isTarifeTrack =
            trackEl.classList.contains( 'tarife' ) &&
            ! trackEl.closest( '.decider' );

        if ( isLatestPostsTrack ) return 'latest';
        if ( isTarifeTrack ) return 'tarife';
        return 'group';
    }
    ...
    function bootstrap() {
        ...
        document
            ...
            .forEach( ( trackEl ) => {
                if ( trackEl.classList.contains( 'tarife' ) && trackEl.closest( '.decider' ) ) {
                    if ( trackEl.classList.contains( 'js-column-slider' ) ) {
                        destroySlider( trackEl );
                    }
                    return;
                }

                const isActive = trackEl.classList.contains( 'js-column-slider' );
                const type = getTrackType( trackEl );

                if ( type === 'latest' ) {
                    if ( ! isActive ) {
                        initSlider( trackEl, type );
                    } else {
                        const inst = instances.find( ( ins ) => ins.track === trackEl );
                        if ( inst ) inst.recalc();
                    }
                    return;
                }
                ...
                if ( type === 'tarife' ) {
                    if ( viewportWidth < tarifeBreakpoint && ! isActive ) {
                        initSlider( trackEl, type );
                    } else if ( viewportWidth >= tarifeBreakpoint && isActive ) {
                        destroySlider( trackEl );
                    } else if ( isActive ) {
                        const inst = instances.find( ( ins ) => ins.track === trackEl );
                        if ( inst ) inst.recalc();
                ...
                if ( viewportWidth <= groupBreakpoint && ! isActive ) {
                    initSlider( trackEl, type );
                } else if ( viewportWidth > groupBreakpoint && isActive ) {
                    destroySlider( trackEl );
                } else if ( isActive ) {
                    const inst = instances.find( ( ins ) => ins.track === trackEl );
                    if ( inst ) inst.recalc();
    ...
    function initSlider( track, type ) {
        ...
        const slides = Array.from( track.children ).filter( ( el ) => {
            ...
            if ( type === 'tarife' ) {
                return el.classList.contains( 'tarife__tarif' );
            }
        ...
        function setByIndex( skipAnim = false ) {
            ...
            if ( skipAnim ) state.track.style.transition = 'none';
        ...
        function recalc() {
            ...
            } else if ( state.type === 'tarife' ) {
                const vw = window.innerWidth;

                // <<< NEU: 3 / 2 / 1 je nach Breite >>>
                let spv = 3;
                if ( vw < 600 ) {
                    spv = 1;
                } else if ( vw < 950) {
        ...
        function getIncrement() {
            if ( state.type === 'latest' ) return state.slidesPerView;
            return 1;
        }
        ...
        function onMove( x, y ) {
            ...
            const overshoot = 60;
            if ( state.currentTranslate > overshoot ) state.currentTranslate = overshoot;
...
document.addEventListener( 'DOMContentLoaded', () => {
    document.querySelectorAll( '.wp-block-table' ).forEach( ( table ) => {
        ...
        updateIndicator();
        table.addEventListener( 'scroll', updateIndicator, { passive: true } );
...
// Tariftabelle: Toggle-Anker
document.addEventListener( 'DOMContentLoaded', function () {
    document.querySelectorAll( '.tariftabelle' ).forEach( function ( table ) {
        table.hidden = true;

        const toggle = document.createElement( 'a' );
        toggle.href = '#';
        toggle.className = 'tariftabelle-toggle';
        toggle.textContent = 'Tarifdetails einblenden';

        toggle.addEventListener( 'click', function ( e ) {
            e.preventDefault();
            const isHidden = table.hidden;
            table.hidden = ! isHidden;
            toggle.textContent = isHidden ? 'Tarifdetails ausblenden' : 'Tarifdetails einblenden';
        } );
